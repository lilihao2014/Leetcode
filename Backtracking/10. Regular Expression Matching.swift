...
class IsMatch {
    func isMatch1(_ s: String, _ p: String) -> Bool {
        ...
        let isFirstMatch = !s.isEmpty && (p[p.index(p.startIndex, offsetBy: 0)..<p.index(p.startIndex, offsetBy: 1)] == "." || p[p.index(p.startIndex, offsetBy: 0)..<p.index(p.startIndex, offsetBy: 1)] == s[s.index(s.startIndex, offsetBy: 0)..<s.index(s.startIndex, offsetBy: 1)])
